      ...
      <tr>
        ...
        <td>
          <select name="gender">
            ...
            <option value="M" <c:if test="${param.gender == 'M'}">selected</c:if>>男性</option>
            <option value="F" <c:if test="${param.gender == 'F'}">selected</c:if>>女性</option>
          </select>
        </td>
      </tr>
      <tr>
        ...
        <td>
          <select name="sort">
            <option value="id"     <c:if test="${param.sort == 'id'}">selected</c:if>>社員ID</option>
            <option value="name"   <c:if test="${param.sort == 'name'}">selected</c:if>>氏名</option>
            <option value="gender" <c:if test="${param.sort == 'gender'}">selected</c:if>>性別</option>
            <option value="note"   <c:if test="${param.sort == 'note'}">selected</c:if>>備考</option>
          </select>
        </td>
      </tr>
      <tr>
        ...
        <td>
          <select name="order">
            <option value="asc"  <c:if test="${param.order == 'asc'}">selected</c:if>>昇順</option>
            <option value="desc" <c:if test="${param.order == 'desc'}">selected</c:if>>降順</option>
          </select>
        </td>
  ...
  <c:choose>
    ...
    <c:otherwise>
      <table>
        ...
        <tbody>
          <c:forEach var="s" items="${shainList}">
            <tr>
              <td>${s.id}</td>
              <td>${s.name}</td>
              <td>
                <c:choose>
                  ...
                  <c:otherwise>女性</c:otherwise>
                </c:choose>
              </td>
              <td>${empty s.note ? "" : s.note}</td>
              <td>
                <a href="shain_hensyuu?id=${s.id}">更新</a> |
                <form action="shain_sakujyo" method="post" style="display:inline;">
                  <input type="hidden" name="id" value="${s.id}">
                  <button type="submit"
                          onclick="return confirm('本当に削除しますか？');"
                          style="background:none; border:none; color:#4caf50; font-weight:bold; cursor:pointer;">
                    削除
                  </button>
                </form>
              </td>
